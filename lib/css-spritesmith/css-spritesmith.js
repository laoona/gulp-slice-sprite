...
function rem2px(value) {
    var baseRem = parseInt(defaultOptions.baseFonts2rem);

    return value / baseRem * 1;
}

function getBgPosCss(x, y, ratio) {
    var rem = defaultOptions.REM;
    var type = rem ? 'rem' : 'px';
...
function cssSpriteSmith(options, callback) {
    ...
    async.waterfall([
        ...
        function buildCss(cssData, cb) {
            ...
            var spriteDataRem = sliceData.spriteData;
            ...
            if (rem) {
                var retinaBgWidthRem = rem2px(spriteDataRem.properties.width);
                var retinaBgHeightRem = rem2px(spriteDataRem.properties.height);

                css += '\n';
                css += 'background-size: ' + retinaBgWidthRem +'rem ' + retinaBgHeightRem + 'rem';
            }
